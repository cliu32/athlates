...
for file in *1.fastq.gz ; do
	...
	mkdir $rsltf/$sample
	cd $rsltf/$sample
	#read mapping using novoalign (Option 1: for exome-seq from 1000 Genomes Project with shorter read length, "-n 100")
	...
	#read mapping using novoalign (Option 2: for amplicon shotgun sequencing data with longer read length, "-n 150")
	#novoalign -o SAM -r Random -i PE 100-1400 -H -t 30 -n 150 -c 12 -d $db/ref/$version.clean.ndx -f $dataf/$sample$suffix1 $dataf/$sample$suffix2 | samtools view -bS -h -F 4 - > $sample.bam
	#generating sorted bam file
	samtools sort $sample.bam -o $sample.sorted.bam
	#run athlates for in silico HLA typing 
	for locus in A B C DRB1 DQB1; do
